...
- [x] 医師側の患者台帳UIで基礎疾患・既往歴を保存し、履歴表示とChatGPT貼り付け用テキストに反映する
- [x] 医師側の患者台帳UIで併存相談チェックを構造化し、患者画面では登録済み背景を表示して変更時だけ補足できるようにする
