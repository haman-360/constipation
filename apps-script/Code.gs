...
function historyTimelineItemHtml_(kind, label, date, summary, fields, editFormHtml, editMode) {
  const fieldHtml = fields
    ...
    .map(([fieldLabel, value]) => `<div><dt>${escapeHtml_(fieldLabel)}</dt><dd>${escapeHtml_(value)}</dd></div>`)
    .join("");
  return {
    ...
    html: `
          <article class="timeline-card timeline-card--${escapeHtml_(kind)}">
            ...
            ${fieldHtml ? `<dl class="timeline-fields">${fieldHtml}</dl>` : ""}
            <details class="timeline-edit" ${editMode ? "open" : ""}>
              <summary>編集</summary>
              ${editFormHtml}
            </details>
          </article>`,
