...
- `date-format-converter.html`
  - TSV内の日付セルを `2025/08/01` から `2025-08-01` へ、`patient_id` を `1234` から `01234` へ一括変換する補助ツールです。
...
- 日付は `YYYY-MM-DD` を基本にします。`YYYY/MM/DD` で入力済みの場合は、`date-format-converter.html` にTSVを貼り付けると一括変換できます。`patient_id` が4桁以下の場合も5桁に補正します。時刻が必要な場合は `YYYY-MM-DD HH:MM` で入力します。
