const els = {
  ...
  openEditButton: document.getElementById("openEditButton"),
  ...
  editUrlOutput: document.getElementById("editUrlOutput"),
  ...
  editOpenLink: document.getElementById("editOpenLink"),
...
function updateOutput() {
  ...
  const editUrl = buildActionUrl("doctorHistory", { edit: "1" });
  ...
  const ready = Boolean(entryUrl && profileUrl && doctorUrl && editUrl && historyUrl && preVisitContextUrl && treatmentContextUrl);

  els.openEntryButton.disabled = !ready;
  els.openProfileButton.disabled = !ready;
  els.openDoctorButton.disabled = !ready;
  els.openEditButton.disabled = !ready;
  ...
  els.editUrlOutput.textContent = editUrl || "患者IDとWeb App URLを入力してください。";
  ...
  els.editOpenLink.classList.toggle("is-disabled", !ready);
...
els.openEditButton.addEventListener("click", () => openUrl(buildActionUrl("doctorHistory", { edit: "1" })));
