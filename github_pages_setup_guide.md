...
3. Google Apps Script Web App URLにデプロイ済みの `/exec` URLが既定値として入っていることを確認する。Web Appを再作成してURLが変わった場合だけ入力し直す。
...
GitHub Pages上のHTMLとJavaScriptは、ブラウザから閲覧できる。そのため、既定値として埋め込んだGoogle Apps Script Web App URLは、秘密鍵ではなく「見える可能性があるURL」として扱う。

運用上の注意:

- Web App URLを院外へ直接共有しない。
- QR URLには `patient_id` と `visit_token` だけを含め、`submit_url` は含めない。
- 患者IDは5桁の院内IDに限定し、氏名などの直接識別情報をURLへ入れない。
- Google Sheetsは必要な医療者だけに共有する。
- 不自然な保存行や重複行がないか、運用初期は定期的に確認する。
- URLが意図せず広がった場合は、Apps Scriptを新しいデプロイとして作り直し、HTML側の既定Web App URLを更新する。
