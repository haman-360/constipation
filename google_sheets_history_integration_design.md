...
- `visits` に1回分の問診・出力を保存する。
- `patients` に同じ `patient_id` がなければ、個人情報を入れずに台帳行を自動追加する。
- 直近日誌入力がある場合は、`diary_weekly` に週次集計として1行追加する。
- `prescriptions` と `toilet_training` は患者回答から自動生成しない。処方履歴とトイレトレーニング状況は医師側入力で管理する。
...
```text
patient_id=12345&limit=5&mode=preVisit
```

`mode`:

- `preVisit`: 診察前整理モード。変化点、不足情報、矛盾点、追加確認事項だけを整理する。
- `treatmentReview`: 医師向け治療方針検討モード。方針候補、推奨度、理由、避けたい方針、再診間隔、保護者説明案を整理する。
...
医師が処方履歴とトイレトレーニング履歴を追記するためのHTMLフォームを表示する。

入力:

```text
action=doctorEntry&patient_id=12345&limit=5
```

この画面から保存する内容:

- `prescriptions`: 処方日時、薬剤名、量、指示内容、医師メモ
- `toilet_training`: 記録日時、トレーニング状況、おむつ・パンツ、トイレ拒否、メモ

処方履歴だけ、トイレトレーニング履歴だけ、両方同時保存の3操作を用意する。保存後は医師入力画面へ戻し、ブラウザ更新で同じ送信を繰り返さない。

`prescriptions.date` と `toilet_training.date` は `yyyy-MM-dd HH:mm:ss` 形式で保存する。

患者問診から推測して自動入力しない。医師が確認した事実だけを追記する。
...
医師が診察で検討するための材料として使う。患者・保護者へ直接指示する文体にはしない。

```text
【医師向け治療方針検討モード】
診断や処方の最終決定は医師が行います。
最新の小児便秘診療ガイドラインや一般的な診療原則と矛盾しにくい方針候補を整理してください。
方針候補、推奨度、理由、避けたい方針、再診間隔、追加確認事項、保護者説明案を整理してください。
```
